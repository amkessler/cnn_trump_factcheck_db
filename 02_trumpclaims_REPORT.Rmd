---
title: "Trump Falsehood Breakdowns"
output:
...
WORKINGTABLE %>% 
  ...
  count(current_party, live_winning) %>% 
    kable() %>% 
...
Key races by current party holding
```{r echo=FALSE}
WORKINGTABLE %>% 
  filter(!is.na(keyrace_rating)) %>% 
  count(keyrace_rating_wcurparty, live_winning) %>% 
    kable() %>% 
    kable_styling(bootstrap_options = c("striped", "condensed"), 
                full_width = F,
                position = "left")
...
```{r echo=FALSE}
  WORKINGTABLE %>% 
    filter(flips == "Y") %>% 
    count(live_winning, pct.ed.college.all.abovebelow.natl) %>% 
    kable() %>% 
    kable_styling(bootstrap_options = c("striped", "condensed"), 
                full_width = F,
                position = "left")
...
```{r echo=FALSE}
  WORKINGTABLE %>% 
    filter(flips == "Y") %>% 
    count(live_winning, medincome.abovebelow.natl) %>% 
    kable() %>% 
    kable_styling(bootstrap_options = c("striped", "condensed"), 
                full_width = F,
                position = "left")
...
```{r echo=FALSE}
  WORKINGTABLE %>% 
    filter(flips == "Y") %>% 
    count(live_winning, medage.abovebelow.natl) %>% 
    kable() %>% 
    kable_styling(bootstrap_options = c("striped", "condensed"), 
                full_width = F,
                position = "left")
