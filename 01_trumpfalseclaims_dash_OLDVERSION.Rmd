...
raw_data <- read_excel("DALE TRUMP FACT CHECKS.xlsx", sheet = "ak",
...
fcheck$category_combined <- str_trim(gsub("[; ]*$", "", fcheck$category_combined))
...
tag5 <- fcheck %>% 
  select(tag = category_tag5)

#combine into one
tag_combined <- bind_rows(tag1, tag2, tag3, tag4, tag5)
...
tag5b <- fcheck_PREVWEEK %>% 
  select(tag = category_tag5)

#combine into one
tag_combined_oneweek <- bind_rows(tag1b, tag2b, tag3b, tag4b, tag5b)

#filter out NAs
tag_combined_PREVWEEK <- tag_combined_oneweek %>% 
  filter(!is.na(tag), tag != "") 
...
zzz <- fcheck %>% 
  count(code_name, category_combined)
...
#manual cleaning and then bringing back in to join
codelisttoclean_joinlist <- read_excel("saved_versions/codelisttoclean_joinlist.xlsx")

codelist_un <- codelisttoclean_joinlist %>% unique()

ztemp <- anti_join(fcheck, codelisttoclean_joinlist)

keeptemp<- inner_join(fcheck, codelisttoclean_joinlist)

alltemp <- bind_rows(keeptemp, ztemp)

recnos <- alltemp %>% 
  count(recno) %>% 
  arrange(desc(n))

write_xlsx(alltemp, "saved_versions/alltemp_codejoins.xlsx")

fcheck <- alltemp

write_xlsx(fcheck, "saved_versions/fcheck_fortransfer.xlsx")

finalcount <- fcheck %>% 
  count(code_name, cleaned_category_combined)

write_xlsx(finalcount, "saved_versions/codelist_forlookup.xlsx")
...
e <- ggplot(data = bysubject_all, aes(x = reorder(tag, -n), y = n)) +
  geom_col(
    # color = "#848484",
    fill = "#f4a8a7") +
  # coord_flip() +
